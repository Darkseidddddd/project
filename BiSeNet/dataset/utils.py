...
def compute_cm_cks_cr(pred, label):
        pred = pred.flatten()
        label = label.flatten()
        cm = confusion_matrix(label, pred, labels=['surfaces','Building', 'LowVegetation', 'Tree', 'Car', 'Clutter'])
        cks = cohen_kappa_score(label, pred)
